...
class IntranetAPIUtils {
  ...
  String _baseUrl = "http://tracking.gpscontrolcolombia.com/login.html";
  ...
  // ignore: avoid_init_to_null
  Future<dynamic> getAndSaveAutologinLink(String redirectUrl,
      {Cookie cookie: null}) async {
    bool success = await this.loginFromRedirectUri(redirectUrl, cookie: cookie);
    if (!success) return null;

    // Check if url is available
    return this._network.get(this._baseUrl + "/admin/autolog").then((res) {
  ...
  Future<dynamic> get(String url) async {
    var cacheManager = await DefaultCacheManager().getSingleFile(url);

    return cacheManager;
  }

  Future<dynamic> registerToProject(
      String url, String name, List<String> team) async {
    Map jsonMap = {};

    if (name != null) jsonMap["title"] = name;
    if (team != null) jsonMap["members"] = team;
  ...
  Future<dynamic> unregisterToProject(
      String url, String projectName, String codeInstance, String email) async {
    Map jsonMap = {};
    String code =
        projectName.replaceAll(' ', '-') + "-" + codeInstance + "-" + email;
